...
my_channel_id = "-1001753457449"
my_channel_id2 = "айди второго канала"
my_channel_id3 = "айди третего и такт далее"
channel_us = "https://t.me/vandam_music"
channel_us2 = "второй"
channel_us3 = "третий ну тоже понятно"
...
def no_sub():
    ...
    urlButton2 = InlineKeyboardButton(text='Канал 2', url=channel_us2)
    urlButton3 = InlineKeyboardButton(text='Канал 3', url=channel_us3)
...
async def ch_sub(sid):
    ...
    y = await bot.get_chat_member(my_channel_id2, sid)
    z = await bot.get_chat_member(my_channel_id3, sid)
    if x.status in statuss:
        if y.status in statuss:
            if z.status in statuss:
                return(1)
            else:
		...
		if user is None:
			...
			if str(referal_id) != '':
				if str(referal_id) != str(user_id):
					...
					try:
						pass # Тут не доделаная рефералка
					except Exception as e:
						print(e)
						pass
				else:
					await register_user(user_id, None)
					await bot.send_message(message.chat.id, f"<b>You can't register with your link</b>")
			else:
				await register_user(user_id, None)
			await bot.send_message(logs, f'<b>New user {message.from_user.get_mention()} / <code>{user_id}</code></b>')

		await message.answer_photo(photo=photo, caption=f'<b>Welcome {message.from_user.get_mention()}. Vandam Market - Best Market</b>', reply_markup=await main_menu(user_id))

@vip.callback_query_handler(text='back', state='*')
async def back_msg(call: types.CallbackQuery):
	await call.message.edit_caption(caption=f'<b>Welcome {call.from_user.get_mention()}. Vandam Market - Best Market</b>', reply_markup=await main_menu(call.from_user.id))
...
async def topup_msg(call: types.CallbackQuery, state: FSMContext):
	...
	await call.message.edit_caption(caption=f'<b>Enter the top up amount in $</b>', reply_markup=await back_profile_menu())
	await Topup.sum.set()
...
async def Topup_sum_state(call: types.CallbackQuery, state: FSMContext):
	...
	try:
		if int(call.text) <= 0:
			await cl.message.edit_caption(caption=f'<b>❗Enter a natural number</b>', reply_markup=await back_profile_menu())
			await state.finish()
		else:
			topup_sum = int(call.text)
			await state.update_data(topup_sum=topup_sum)
			await cl.message.edit_caption(caption=f'<b>❕Choose payment currency</b>', reply_markup=await payment_currency_menu())
			await Topup.currency.set()
	except:
		await cl.message.edit_caption(caption=f'<b>❗ Enter only numbers</b>', reply_markup=await back_profile_menu())
		await state.finish()
...
async def crypto_bot_currency_msg(call: types.CallbackQuery, state: FSMContext):
	...
	await call.message.edit_caption(caption=f'<b>🪐 Top Up Market\n\nAmount: {topup_sum}\nСryptocurrency: {call.data.split("|")[1]}\n\n❕Pay your bill using the link below</b>', reply_markup=await check_crypto(invoice.pay_url, invoice.invoice_id, topup_sum))
	await state.finish()
...
async def check_crypto_bot_funds(call: types.CallbackQuery):
	try:
		if await check_crypto_bot_invoice(int(call.data.split('|')[1])):
			user_id = call.from_user.id
			user = await get_user(user_id)
			ammot = int(user[1]) + int(call.data.split('|')[2])
			await edit_user_balanse(int(user_id), int(ammot))
			await bot.answer_callback_query(call.id, f'✅ Invoice paid successfully + {call.data.split("|")[2]}$', True)
			await call.message.edit_caption(caption=f'<b>Welcome {call.from_user.get_mention()}. Vandam Market - Best Market</b>', reply_markup=await main_menu(call.from_user.id))
			...
			ammot = int(stats[0]) + int(call.data.split("|")[2])
			await add_stats_profit(ammot)
		else:
			await bot.answer_callback_query(call.id, '❗ Payment has not been made', True)
	except Exception as e:
		print(e)

@vip.callback_query_handler(text='information')
async def information_msg(call: types.CallbackQuery):
	await call.message.edit_caption(caption='<b>🏝 Information Vandam Market\n\n❔ This is a bot for selling various materials, all goods are valid and working. All questions about the replacement only with proofs.</b>', reply_markup=await information_menu())
...
async def subcategory_msg(call: types.CallbackQuery, state: FSMContext):
	...
	if len(tovars) == 0:
		await bot.answer_callback_query(call.id, '❗ The product is out of stock', True)
	else:
		await call.message.edit_caption(caption=f'<b>Enter quantity</b>', reply_markup=await back_menu())
...
async def edit_subcat_name_state(message: types.Message, state: FSMContext):
	try:
		...
		price = int(data['price']) * int(ammot)
		user = await get_user(message.from_user.id)
		user_balance = int(user[1])
		if int(ammot) > int(len(tovars)):
			await bot.send_message(message.chat.id, '<b>❗The product is out of stock</b>')
			await state.finish()
		elif int(ammot) <= 0:
			await bot.send_message(message.chat.id, '<b>❗Enter a natural number</b>')
			await state.finish() 
		else:
			if user_balance < price:
				await bot.send_message(message.chat.id, "<b>❗You don't have enough funds, replenish the balance in your profile</b>")
				await state.finish()
			else:
		...
		print(e)
		await bot.send_message(message.chat.id, text='<b>❗ Enter only numbers</b>')
		await state.finish()
...
async def add_subcategory_price_state(message: types.Message, state: FSMContext):
	...
	await add_subcategory(data['name'], data['description'], int(price), id, data['type'])
	await message.answer_photo(photo=photo, caption=f'<b>✔️ Add successful</b>', reply_markup=await amdin_menu())
	await state.finish()
...
async def adm_subcategories_add_product_msg(call: types.CallbackQuery, state: FSMContext):
	...
	await call.message.edit_caption(caption=f'<b>➕ Send the product, each on a new 2 line</b>', reply_markup=await back_menu_adm())
...
async def edit_balance_state(call: types.CallbackQuery, state: FSMContext):
	...
	await bot.delete_message(chat_id=call.from_user.id, message_id=call.message_id)
	try:
		await edit_user_balanse(user_id, call.text)
		await cl.message.edit_caption(caption=f'<b>✔️ Balance changed successfully</b>', reply_markup=await back_menu_adm())
		await state.finish()
	except:
		await cl.message.edit_caption(caption=f'<b>❗ You entered not a number</b>', reply_markup=await back_menu_adm())
		await state.finish()
